...
public class LiteShape implements Shape, Cloneable {
	...
	/**
	 * Tests if the interior of the <code>Shape</code> entirely contains the specified
	 * <code>Rectangle2D</code>. This method might conservatively return <code>false</code> when:
	 * <ul>
	 * <li>the <code>intersect</code> method returns <code>true</code> and</li>
	 * <li>the calculations to determine whether or not the <code>Shape</code> entirely contains the
	 * <code>Rectangle2D</code> are prohibitively expensive.</li>
	 * </ul>
	 * This means that this method might return <code>false</code> even though the
	 * <code>Shape</code> contains the <code>Rectangle2D</code>. The <code>Area</code> class can be
	 * used to perform more accurate computations of geometric intersection for any
	 ...
	 * <code>Shape</code> for the entire rectanglar area to be considered contained within the
	 * <code>Shape</code>.
	 * <p>
	 ...
	 * <li>the <code>intersect</code> method returns <code>true</code> and</li>
	 * <li>the calculations to determine whether or not the <code>Shape</code> entirely contains the
	 * rectangular area are prohibitively expensive.</li>
	 * </ul>
	 * This means that this method might return <code>false</code> even though the
	 * <code>Shape</code> contains the rectangular area. The <code>Area</code> class can be used to
	 ...
	 * @return <code>true</code> if the interior of the <code>Shape</code> entirely contains the
	 *         specified rectangular area; <code>false</code> otherwise or, if the
	 *         <code>Shape</code> contains the rectangular area and the <code>intersects</code>
	 ...
	 * {@link Rectangle2D} is the smallest bounding box that encloses the <code>Shape</code>, only
	 * that the <code>Shape</code> lies entirely within the indicated <code>Rectangle2D</code>. The
	 ...
	 * access to the geometry of the <code>Shape</code> outline. If an optional
	 * {@link AffineTransform} is specified, the coordinates returned in the iteration are
	 * transformed accordingly.
	 * <p>
	 * Each call to this method returns a fresh <code>PathIterator</code> object that traverses the
	 * geometry of the <code>Shape</code> object independently from any other
	 * <code>PathIterator</code> objects in use at the same time.
	 * </p>
	 * <p>
	 * It is recommended, but not guaranteed, that objects implementing the <code>Shape</code>
	 ...
	 * </p>
	 * <p>
	 * Before using a particular implementation of the <code>Shape</code> interface in more than one
	 * thread simultaneously, refer to its documentation to verify that it guarantees that
	 ...
	 *            are returned in the iteration, or <code>null</code> if untransformed coordinates
	 *            are desired
	 ...
	 * </p>
	 * <p>
	 * If an optional <code>AffineTransform</code> is specified, the coordinates returned in the
	 * iteration are transformed accordingly.
	 * </p>
	 * <p>
	 * The amount of subdivision of the curved segments is controlled by the <code>flatness</code>
	 * parameter, which specifies the maximum distance that any point on the unflattened transformed
	 * curve can deviate from the returned flattened path segments. Note that a limit on the
	 * accuracy of the flattened path might be silently imposed, causing very small flattening
	 ...
	 * <p>
	 * Each call to this method returns a fresh <code>PathIterator</code> object that traverses the
	 * <code>Shape</code> object geometry independently from any other <code>PathIterator</code>
	 ...
	 * <code>Rectangle2D</code>. This method might conservatively return <code>true</code> when:
	 * <ul>
	 * <li>there is a high probability that the <code>Rectangle2D</code> and the <code>Shape</code>
	 * intersect, but</li>
	 * <li>the calculations to accurately determine this intersection are prohibitively
	 * expensive.</li>
	 * </ul>
	 * This means that this method might return <code>true</code> even though the
	 * <code>Rectangle2D</code> does not intersect the <code>Shape</code>.
	 *
	 * @param r the specified <code>Rectangle2D</code>
	...
	private Geometry createRectangle(double x, double y, double w, double h) {
		Coordinate[] coords = {new Coordinate(x, y), new Coordinate(x, y + h), new Coordinate(x + w, y + h), new Coordinate(x + w, y),
				new Coordinate(x, y)};
